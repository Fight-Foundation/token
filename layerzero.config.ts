...
const bscContract: OmniPointHardhat = {
    ...
    contractName: 'FightOFTSecondary', // Corrected FIGHT Token OFT contract (0 initial supply)
}
...
export default async function () {
    ...
    const connections = await generateConnectionsConfig([
        [
            bscContract, // Chain A contract (BSC Mainnet)
            solanaContract, // Chain B contract (Solana Mainnet)
            // Production Security: Multiple DVNs for decentralized verification
            [
                ['LayerZero Labs', 'Google', 'Nethermind'], // required DVNs (3/3 threshold)  
                [['Horizen'], 1], // optional DVNs with threshold
            ],
            [20, 15], // [BSC to Solana confirmations, Solana to BSC confirmations] 
